...
Authorization and per-record ability checks match complete identity tuples and batch candidate records within driver query limits. Attachment handles use canonical backing-record ownership returned by the server, including unprojected lifecycle records, while retaining the originating resource name for scoped authorization; direct attachment member lookups authorize against every configured resource alias backed by the owner record type and declaring the attachment. Resource aliases can therefore expose a composite identity even when attachment storage uses a different backing key. WebSocket lifecycle publishers fan out through every resource backed by a record class; each resource receives its own configured identity, and key-changing updates include the previous identity so remote instance listeners move to the new tuple before later events arrive. Filtered subscriptions receive authorized key-changing updates even when the updated record no longer matches their query, but the empty matched-filter list keeps their callbacks from firing while instance listeners move to the new identity. Create/update records are reloaded and serialized through the subscribed resource, so an alias never receives backing-model attributes it does not expose.
