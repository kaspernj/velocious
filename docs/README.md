...
- `docs/application-process-lifecycle.md`: Initializer startup/teardown, immutable process contexts, promise/race semantics, and process-owner cleanup boundaries.
- `docs/attachments.md`: Backend record attachment inputs, storage drivers, synchronized client policy, path allowlisting/streaming, and persistence lifecycle.
...
- `docs/frontend-tenant-sqlite-lifecycle.md`: Bounded immutable tenant SQLite handles, migration readiness, persistence, eviction safety, and browser/Expo proof coverage.
