...
- **Readiness re-announcement.** The main removes a worker from its ready set on each dispatch and only re-adds it when the worker sends a fresh `ready`. A worker therefore re-announces free capacity on *every* completion that leaves it below its cap (not only on the single at-cap→below-cap edge), and re-announces spare capacity on each heartbeat as a safety net. This means a single missed or dropped `ready` message can't leave a worker stranded out of the ready set and silently freeze dispatch — a failure mode that surfaced under bursty high-concurrency load.
