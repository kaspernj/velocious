...
- Caps are config-driven and tunable. Adding, removing, or changing `queues[name].maxConcurrent` is reconciled against the existing backlog when the main process starts: persisted jobs adopt or release the queue key to match the current config, so a changed cap takes effect without waiting for the queue to drain. Startup reconciliation is serialized across processes with a database advisory lock, so concurrently starting mains cannot interleave the adoption/release UPDATEs. Schema/tenant checks (`db:migrate`, `db:tenants:*`) and routine store/connection initialization never run this reconciliation — they stay read-only regarding queued job rows, so concurrent tenant initialization cannot deadlock against active job processes.
