...
Pooled jobs also persist acceptance evidence so a `handed_off` row can be told apart from one whose runner never picked it up: `child_received_at_ms` records when the executing child's event loop processed the job message, `child_started_at_ms` records when its perform started, and `child_instance_id` / `child_pid` identify the child process that accepted it. The child reports both observations to the worker over IPC carrying the exact handoff lease, and the worker persists them through a fenced store update, so a report that lands after the job was reclaimed or re-handed off is dropped instead of stamping the wrong attempt. The evidence is cleared whenever the lease is invalidated (new handoff, reschedule, return to queue, failure retry) and kept on terminal outcomes as historical evidence for the lost attempt. These fields are diagnostic only — completion and failure still flow through the runner's durable status reporter.
...
Each batch first `SELECT`s a bounded page of candidate ids, then deletes exactly that page with `DELETE ... WHERE id IN (...)`, so a large backlog is removed incrementally rather than in one long transaction. Discovery and deletion are split by the count-revision lock: candidate pages are selected **outside** the singleton `background_job_count_revisions` write lock, and the short serialized transaction that follows only revalidates the selected ids (still the same terminal status and still past the cutoff), deletes the survivors, and publishes the count delta from the database's actual affected-row count. A slow or large retention scan therefore never holds the global enqueue/count mutation lock, and enqueue acknowledgements are unaffected by sweep duration. Pages are ordered deterministically (terminal timestamp ascending, then id ascending), so repeated sweeps converge on the same page sequence. When a concurrent pruner removes some candidates first, the current pass continues to the next page instead of declaring convergence from a smaller affected-row count; the serialized delete transaction and the durable count-revision row are the only shared serialization points between processes, and each batch runs on its own connection so the sweep yields between batches.

To keep discovery bounded as the table grows, readiness ensures three additive composite indexes on `background_jobs`, one per terminal status: `(status, completed_at_ms, id)`, `(status, failed_at_ms, id)`, and `(status, orphaned_at_ms, id)`, named `index_background_jobs_completed_retention`, `index_background_jobs_failed_retention`, and `index_background_jobs_orphaned_retention`. A versioned framework migration creates any missing index idempotently under a schema advisory lock and records itself only after all three exist; routine readiness never rebuilds or drops them.

Retention only ever deletes terminal rows; `queued` and in-flight jobs are never pruned. Durable idempotency ownership, mail-operation rows, and stable-schedule order high-water marks are independent of job retention and are not deleted by this sweep.
