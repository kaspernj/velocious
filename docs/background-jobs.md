...
Composite `(status, terminal_timestamp, id)` indexes can further bound discovery as the table grows. They are not installed by routine readiness in this release: MariaDB online index DDL makes concurrent metadata inspection fail with error 1684, and older serving generations inspect the jobs schema before enqueue. Index activation therefore requires a later, separately qualified release after those generations have drained; it must not be folded into activation of this lock-scope fix.
