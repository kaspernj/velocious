...
- `await client.replaceIdentity({resetScopes, cleanup?, replace, subscribeUserScope?})` drains the old generation, atomically purges app-owned rows and rotates the selected scope identities/cursors, invokes `replace`, then optionally subscribes the new user scope. New pull/start/subscribe work waits behind the transition, and old work cannot advance a rotated cursor.
- Mutation queueing is identity-bearing work and therefore does **not** wait through a transition. An explicit `queue()` call that starts after a lifecycle transition receives `SyncClientLifecycleAbortError` before anything is persisted. A tracked-model callback captures its lifecycle generation when the model changes; if its deferred `afterCommit` runs after that generation starts retiring, Velocious suppresses that old-generation queue append instead of replaying it with replacement credentials. App cleanup remains responsible for purging the retiring identity's local cache rows.
...
- A caller awaiting interrupted work receives `SyncClientLifecycleAbortError`. Framework-owned background replay and reconnect pulls suppress only that expected cancellation; transport, cleanup, apply, and other unexpected errors still report or reject normally. Realtime callbacks retain the subscription generation that created them, so a late resume or message from a closed subscription is ignored and cannot join (or deadlock) the transition drain.
...
- The scope carries `resourceTypes` — the types the client can apply (every resource with pull `attributes`). It is a **delivery filter, not an authorization**: it only narrows what the app already allows. It matters because a user-scope subscription is re-checked **per delivery** (below), and that check costs a database query per matched broadcast per subscribed device — so a broadcast of a type this client cannot even apply must be dropped by the cheap type check, never carried into it.
- The server authorizes the scope through the app sync resource's existing `authorizeChanges({params, scope})` (the app decides whether user scopes are allowed, and `scope.resourceType === null` identifies the user scope), and re-checks record access **per delivery** at broadcast fan-out: a user-scope subscription matches a broadcast of any type it declared (a broadcast carrying no type at all never matches), and each published change is then filtered through the sync resource's `changeDeliverable`, which reuses the app's `scopeChangesQuery` ability scoping. `changeDeliverable` receives the **complete broadcast sync entry** — the immutable sync-row `id`, actor-specific metadata, and every other publisher field — with `resourceId`/`resourceType` normalized to strings (on a copy; the published entry is never mutated), so an override can authorize concurrent targeted and shared broadcasts for the same resource identity independently by their exact-row identity; overrides that only read `resourceId`/`resourceType` keep working unchanged. Within the declared types, what bounds a user scope is that access check, not the type. Two users with disjoint access each receive only their own changes over the one connection.
