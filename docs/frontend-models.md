...
- Invalid client query descriptors, such as unknown `select`, `where`, `search`, `joins`, `group`, `sort`, `pluck`, or Ransack attributes, are frontend-model validation errors. They return the specific validation message and `velocious.code: "frontend-model-validation"` instead of being emitted as framework errors.
