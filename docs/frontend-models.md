...
- Frontend models expose a narrower query surface (mainly `find`, `findBy`, `findByOrFail`, `toArray`, `where`, `sort`, `order`, `group`, `count`, `limit`, `offset`, `page`, `perPage`) and do not expose the full backend query API (`joins`, `distinct`, `findOrCreateBy`, etc.).
