...
- Generated model classes carry their concrete lifecycle types into `onCreate`, `onUpdate`, and `onDestroy`: callback models retain the generated class type, scalar event ids are strings, and composite event ids are objects keyed by the configured primary-key attributes.
