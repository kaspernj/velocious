...
- Legacy shared index requests may send `payload.authenticationToken`. The controller extracts only that compatibility field into request context before validating index query keys; an explicit `requestContext.authenticationToken` takes precedence when both are present. `authenticationToken` is not an index query option, and other unknown index payload keys remain rejected.
...
- Invalid client query descriptors, such as unknown `select`, `where`, `search`, `joins`, `preload`, `group`, `sort`, `pluck`, or Ransack attributes, are frontend-model query errors. Shared index payloads also reject unknown top-level keys per request, including wrapper nesting such as `payload.query.where`; use `payload.where` directly. These errors return the specific query error message and `velocious.code: "frontend-model-query-error"` instead of being emitted as framework errors.
...
- `first()` and `last()` are available on frontend query/model classes. An explicitly set `order(...)` is preserved; the deterministic identity order is applied only when no order was set.
