...
`optimisticVersion` and `serverWins` are the only backend-routed strategies currently accepted. A conflict is checked only for routed updates with a non-null `baseVersion` and an existing record. The service serializes concurrent checks per resource identity with an advisory lock. A mismatch returns `syncState: "conflict"` with `affectedFields`, `baseVersion`, `localMutation`, `serverModel`, `serverVersion`, `suggestedResolution`, and `versionAttribute`; it does not apply, persist, publish, or run the after-apply tail. `serverModel` is deliberately partial: it contains identity, version, and authoritative serialized values for affected fields that passed `writableAttributes` and are declared readable in resource `attributes`. When `attributes` is not declared (or is empty), the framework derives the readable set from the model's full database-backed attribute map, matching the default "expose all" contract. Resource attribute serializers are honored. Accepted database-column aliases remain in `affectedFields` and `localMutation`, and a configured version alias remains in `versionAttribute`, while `serverModel` uses canonical model attribute names for affected values, identity, and version so generated frontend models can apply `keep_server` through their canonical accessors. Unaffected, writable-but-hidden, unpermitted, and resource-private fields are omitted. For `serverWins`, merge that partial snapshot locally to converge without another backend read for the returned fields.
