...
Frontend SQLite applications can add a bounded resident lifecycle around the captured identity; see [frontend tenant SQLite lifecycle](frontend-tenant-sqlite-lifecycle.md). That lifecycle migrates each physical database ledger and binds model metadata/readiness to its physical identity and schema generation. `handle.liveQuery(...)` builds sources whose execution and record-change subscription retain that physical identity, and `new SyncClient({tenantHandle: handle, databaseIdentifier})` binds all local sync state and reconnect work to it. Neither API falls back to ambient tenant state.
