...
Frontend SQLite applications can add a bounded resident lifecycle around the captured identity; see [frontend tenant SQLite lifecycle](frontend-tenant-sqlite-lifecycle.md). Per-tenant migration or model-metadata caches, tenant-aware live-query/event routing, and automatic tenant-scoped `SyncClient` construction remain outside this layer.
