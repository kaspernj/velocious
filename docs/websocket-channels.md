...
- The host creates queued persistence outside the publisher's database connection and revocable test-attempt contexts. Persistence therefore owns its checkout and cannot retain a completed request/test transaction or become invalid merely because the originating attempt ends.
...
- `await client.close()` is a final, graceful shutdown: every channel subscription still tracked by the client, including a pending subscription, becomes closed and receives `onClose("client_close")` before the client sends a normal WebSocket close and permanently releases the server session. A pending `subscription.ready` still rejects because no `channel-subscribed` acknowledgement arrived; its error identifies `client_close` as the reason.
- A throwing channel close callback does not prevent the remaining tracked subscriptions, socket, or base client state from closing. After cleanup finishes, `client.close()` rejects with the original error when one callback or teardown step failed, or with an `AggregateError` when multiple steps failed.
- Unexpected transport drops remain eligible for grace-period session resumption.
- With multiple HTTP workers, the standard client adds its prior session id to the reconnect upgrade URL as `velociousSessionId` before it sends the existing `session-resume` message. The host uses that identity only to route the upgrade to the worker holding the paused session; ordinary HTTP sockets and separate sessions from one source IP remain round-robin. Ownership entries exist only for live or grace-paused sessions and are removed on final close, grace expiry, worker replacement, or server stop. A custom wire client that implements resumption must add the same query parameter to its reconnect URL; source-address affinity is not part of the protocol.
