...
- `await client.close()` is a final, graceful shutdown: it sends a normal WebSocket close and permanently releases the server session and its subscriptions. Unexpected transport drops remain eligible for grace-period session resumption.
