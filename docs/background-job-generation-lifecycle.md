...
An executing job carries its exact durable handoff identity through its asynchronous call chain. A public `performLater()` or `performLaterWithOptions()` call made by that job after its generation retires is accepted only while the retired generation still owns the exact producing job, handoff, worker, and handoff timestamp. Producer validation and follow-up insertion or queued deduplication share one store transaction. The retired main acknowledges only after commit and publishes the normal queue wake; it never schedules, drains, dispatches, or hands off the new row. The active generation owns dispatch, including rediscovery of future work after another generation retires.
