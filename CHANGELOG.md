...
- Make attachment input normalization Expo-safe by lazy-loading Node `fs/promises` and `path` only in Node-only branches (`UploadedFile` temp-path reads and `{path: ...}` inputs), so Metro eager bundling no longer resolves `fs/promises` from shared modules.
