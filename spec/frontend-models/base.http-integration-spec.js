...
class User extends FrontendModelBase {
  ...
  static resourceConfig() {
    return {
      ...
      collectionCommands: ["currentSessionCookie", "setSessionCookie", "lookupByEmail", "delayedLookupByEmail", "echoMessage", "rejectDomainOperation"],
      modelName: "User",
      primaryKey: "id"
    }
  }
...
class Task extends FrontendModelBase {
  /**
   * @returns {{attributes: string[], builtInCollectionCommands: string[], builtInMemberCommands: string[], primaryKey: string}}
   */
  static resourceConfig() {
    return {
...
describe("Frontend models - base http integration", {databaseCleaning: {transaction: false, truncate: true}}, () => {
  ...
  it("preserves safe custom-command error fields over real Node HTTP requests", async () => {
    await Dummy.run(async () => {
      configureNodeTransport()
