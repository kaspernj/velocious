...
describe("Background jobs - worker resilience", {databaseCleaning: {truncate: true}}, () => {
  ...
  it("retires a completed same-id queue tracker synchronously before a replacement queue is visible", async () => {
    const {server, worker} = await startConfiguredWorker({})

    worker._runPooledJob = () => Promise.resolve()

    try {
      const payloadA = /** @type {import("../../src/background-jobs/types.js").BackgroundJobPayload & {id: string}} */ ({id: "same-id-1", jobName: "TestJob", args: [], options: {}})

      worker._queuePooledJob(payloadA)

      expect(worker.inflightPooledJobs.size).toEqual(1)
      expect(worker.pooledJobQueueTrackers.has("same-id-1")).toEqual(true)

      const trackerA = worker.pooledJobQueueTrackers.get("same-id-1")
      await trackerA

      expect(worker.inflightPooledJobs.size).toEqual(0)
      expect(worker.pooledJobQueues.has("same-id-1")).toEqual(false)
      expect(worker.pooledJobQueueTrackers.has("same-id-1")).toEqual(false)

      const payloadA2 = /** @type {import("../../src/background-jobs/types.js").BackgroundJobPayload & {id: string}} */ ({id: "same-id-1", jobName: "TestJob", args: [], options: {}})

      worker._queuePooledJob(payloadA2)

      expect(worker.inflightPooledJobs.size).toEqual(1)
      expect(worker.pooledJobQueueTrackers.size).toEqual(1)
      expect(worker.pooledJobQueueTrackers.has("same-id-1")).toEqual(true)

      const trackerA2 = worker.pooledJobQueueTrackers.get("same-id-1")
      await trackerA2

      expect(worker.inflightPooledJobs.size).toEqual(0)
      expect(worker.pooledJobQueues.has("same-id-1")).toEqual(false)

      const payloadB = /** @type {import("../../src/background-jobs/types.js").BackgroundJobPayload & {id: string}} */ ({id: "other-id-1", jobName: "TestJob", args: [], options: {}})

      worker._queuePooledJob(payloadB)

      expect(worker.inflightPooledJobs.size).toEqual(1)
      expect(worker.pooledJobQueues.size).toEqual(1)
      expect(worker.pooledJobQueues.has("other-id-1")).toEqual(true)

      const trackerB = worker.pooledJobQueueTrackers.get("other-id-1")
      await trackerB
