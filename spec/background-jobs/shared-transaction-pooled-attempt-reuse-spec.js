...
let firstAttemptPid
/** @type {ReturnType<typeof createBackgroundJobUpdateObserver> | undefined} */
let updateObserver
...
describe("Background jobs - pooled broker attempt reuse", {tags: ["dummy"], databaseCleaning: {transaction: true, truncate: false}}, () => {
  ...
  it("uses dispatch-time broker coordinates in a child created before broker activation", async () => {
    if (!backgroundJobs || !updateObserver) throw new Error("Expected background jobs to be started")
    ...
    const update = await updateObserver.waitForUpdate(jobId)
  ...
  it("runs concurrent jobs sharing the new attempt capability in the reused process", async () => {
    if (!backgroundJobs || !updateObserver || firstAttemptPid === undefined) throw new Error("Expected first pooled attempt to complete")
    ...
    const updates = await Promise.all(jobIds.map(async (jobId) => await updateObserver.waitForUpdate(jobId)))
    expect(updates).toEqual(jobIds.map((jobId) => ({accepted: true, jobId, status: "completed"})))
    const results = await Promise.all(outputPaths.map(async (outputPath) => await waitForOutputJson({outputPath})))
