...
async function createTempProjectDir() {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "velocious-runner-"))
...
describe("Cli - Commands - runner", () => {
  it("imports the provided file path and runs its default function with initialized DB context", {databaseCleaning: {transaction: false, truncate: false}}, async () => {
    ...
    try {
      await fs.writeFile(path.join(directory, "scripts", "custom-runner.js"), `export default async function customRunner({args, db, dbs}) {
...
`)
      ...
      const cli = new Cli({
        ...
        processArgs: ["runner", "scripts/custom-runner.js", "hello", "world"],
        testing: true
      })
