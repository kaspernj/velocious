...
async function createTempProjectDir() {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "velocious-db-seed-"))

  await fs.mkdir(path.join(directory, "src", "db"), {recursive: true})
...
describe("Cli - Commands - db:seed", () => {
  it("imports src/db/seed.js and runs its default function with initialized DB context", {databaseCleaning: {transaction: false, truncate: false}}, async () => {
    const {directory, cleanup} = await createTempProjectDir()

    try {
      await fs.writeFile(path.join(directory, "src", "db", "seed.js"), `export default async function runSeed({args, configuration, db, dbs}) {
...
`)
      ...
      const cli = new Cli({
        ...
        processArgs: ["db:seed", "first", "second"],
        testing: true
      })
