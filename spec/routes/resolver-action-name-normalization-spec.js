...
describe("routes - resolver action name normalization", {databaseCleaning: {transaction: false, truncate: false}}, async () => {
  it("maps slash-based route actions to camelized controller methods", async () => {
    ...
    const requestLines = [
      "POST /current-user/update HTTP/1.1",
      "Host: example.com",
      "",
      ""
    ].join("\r\n")
    ...
    expect(JSON.parse(response.getBody())).toEqual({message: "Current user updated"})
  })
})
