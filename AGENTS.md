...
## Background-jobs release lifecycle contract

Framework changes and documentation must preserve this production invariant. A
release generation contains its own `background-jobs-main` and worker pool. The
complete candidate generation starts before activation. After activation the old
main and workers retire as one unit: the main stops schedule ownership, new
dispatch, and new worker handoffs, while workers stop accepting handoffs. The old
main MUST remain on its old endpoint with its old workers and continue owning
connections/heartbeats, acknowledgements, durable report retries, per-job
timeouts, child reaping, and durable transitions until every handoff settles and
every worker exits. Returned/retried queue work is eligible for the active
generation; the retired main never dispatches it again. Old workers MUST NOT
reconnect or hand over to the new main during normal deploy draining.

Deploy success/lock release follows healthy candidate activation and MUST NOT
wait for retired jobs generations, workers, jobs, HTTP/WebSocket connections, or
other retained services. Multiple generations may drain for hours under durable
supervision across later deploys and recovery, with their releases pinned against
cleanup. HTTP drain completion/timeout never kills a jobs generation.
Runtime-owner replacement preserves or transfers supervision and returns once
healthy; it is not full synchronous shutdown. Beacon may stay shared on `7330`,
but jobs-main uses release-scoped endpoints. Per-job timeouts remain valid;
worker-shutdown timeout is never the primary deploy mechanism.

Do not describe the current framework as fully compliant while normal-deploy
worker adoption or missing retired-main quiescence contradicts this contract.

Terminal-job retention must discover bounded candidate IDs without the singleton
count-revision mutation lock. The serialized transaction may only revalidate and
delete the selected IDs and publish the actual count delta. Retention scans must
use deterministic status/timestamp indexes and must never be "fixed" by raising
the enqueue acknowledgement timeout or disabling retention.

## Build output
`build/` contains compiled files generated by `npm run build` and is shipped in Git/npm artifacts. Do not edit files under `build/` manually. After changing runtime source, run `npm run build` before the final validation and commit the resulting `build/` changes in the same commit/PR. After merging or resolving conflicts that touch runtime source or `build/`, rerun `npm run build` and commit any regenerated drift. Never resolve `build/` conflicts with a blanket `ours`/`theirs` merge rule because that can silently ship stale runtime code.

## Docker development environment
The checked-in root `Dockerfile` and `compose.yml` define one canonical `dev` service used by humans, CI, and agent systems alike. The image is Ubuntu 26.04 LTS (pinned by the approved digest) with Node.js 24.x from signed NodeSource (verified key checksum), the universal apt coding/debugging baseline, the newest published retained provider CLIs installed from bare unversioned npm specs, and owner-pinned native Qwen Code 0.23.3. It is source-independent — no project source is copied and no project dependencies are installed at image build time; keep it that way.

- Canonical artifact names only: root `Dockerfile` and `compose.yml`. Never add Dockerfile/Compose basename variants named after providers, orchestrators, workers, or tasks.
- The service runs as UID/GID 1000 (the image renames Ubuntu's existing `ubuntu` identity to `dev`) with `HOME=/home/dev` and working directory `/home/dev/velocious`; there is no container-side chown/root fixup or host UID/GID synchronization.
- The dev service mounts exactly four long-form binds with host-path creation disabled: the complete `${DEV_HOME_PATH:-/home/dev}` development home at `/home/dev`, `${AI_PROVIDER_RUNTIME_SOURCE_PATH}` read/write at `/opt/hermes-dind-shared/auth/provider-runtime`, one exact immutable `${AGENT_CONTEXT_SOURCE_PATH}` read-only at `/opt/hermes-agent-context`, and `${GH_CONFIG_SOURCE_PATH}` read-only at `/home/dev/.config/gh`. Provider runtime and agent context sources must live outside the development home; never mount mutable `current`, `/opt/data`, broad shared roots, SSH keys, npm credentials, or additional credential paths.
- `scripts/bootstrap-provider-runtime.sh` owns provider discovery links. It requires `.local` and `.local/share` to be real runtime directories, then validates but never creates, migrates, or replaces the canonical runtime aliases `.codex -> codex`, `.kimi-code -> kimi-code`, `.opencode -> opencode`, and `.local/share/opencode -> ../../opencode`. Shared provider-link validation/migration is serialized before lane-local setup. Codex, Kimi, and legacy OpenCode homes use those shared provider directories; OpenCode config/state/cache remain lane-local except for the exact shared `auth.json`. Conflicting managed lane paths and discovery links are migrated to timestamped backups without deletion, chown, or permission broadening.
- Threadwire is not installed in the image or the project; it remains parent orchestration resolved through unversioned `npx` outside the container. The dev environment points Threadwire's Codex, Kimi, and OpenCode executable overrides directly at the installed provider binaries.
- Instance isolation uses the standard Compose project-name contract: `compose.yml` declares the top-level `name: velocious` default, and `COMPOSE_PROJECT_NAME` (or `-p`) plus a distinct `DEV_HOME_PATH` runs an isolated instance.
- Install dependencies with the normal package commands inside the running service (for example `docker compose exec dev npm ci`); `scripts/docker-run.sh <command>` runs a one-off command in a disposable container.
- After changing any Docker artifact, run `npm run verify:docker-dev-environment` (the checked-in static contract verifier, including its negative probes) before committing.
...
Keep each test/spec file to one top-level `describe`, and keep files small and focused; split them by behavior before they grow large.

### Model lifecycle callbacks

Both implicit instance lifecycle methods and explicit static registrations are supported. Keep an implicit hook only when it is one small, cohesive model-owned operation; use multiple explicit named registrations for independent responsibilities or visible ordering. Registered callbacks are class-level and ordered, so register them deterministically during model initialization, never repeatedly in request or runtime paths. `afterSave` is pre-commit; put irreversible effects behind `afterCommit`. Do not confuse record lifecycle callbacks with frontend-model resource hooks. Read [the record lifecycle callback guide](docs/lifecycle-callbacks.md) before adding or refactoring hooks.
...
- Database-backed specs use transaction cleaning by default and must keep it enabled. Never set `databaseCleaning: {transaction: false, truncate: true}` on a suite or individual example to work around isolation, connection-checkout, or MSSQL constraint-toggle failures: truncating every table disables and re-enables all constraints per test and destroys suite performance. Use the test runner's shared transaction connection for in-process requests and fix the actual connection/lifecycle ownership instead. A spec that genuinely never touches the configured databases, or that explicitly owns/restarts the pool lifecycle it is testing, may use `{transaction: false, truncate: false}` to skip the configured testing hook's cleaning entirely, but truncation is not a fallback for transaction-compatible coverage.
...
`@velocious/testing` is the sole owner of generic expectations, matchers, equality, asymmetric matching, promise matching, mocks, and change observation. Velocious compatibility paths may only re-export or delegate that behavior; keep only application, database, model, resource, and runner lifecycle integration in this repository.
