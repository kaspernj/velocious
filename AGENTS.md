...
## Build output
`build/` contains checked-in compiled files generated by `npm run build`. Do not edit files under `build/` manually. Regenerate and commit it whenever source changes affect compiled output so pinned Git archives work without lifecycle scripts.
