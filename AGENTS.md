...
- The dev service mounts exactly two binds: the complete `${DEV_HOME_PATH:-/home/dev}` development home at `/home/dev` (the checkout is expected at `$DEV_HOME_PATH/velocious`) and the required `${GH_CONFIG_SOURCE_PATH}` GitHub CLI config read-only at `/home/dev/.config/gh` with container-side `GH_CONFIG_DIR` pointing there. The GitHub CLI config is the sole authorized credential mount — do not add SSH keys, provider credentials, or other secret mounts to the tracked Compose files. Kimi and other provider credentials are an external operational override owned by the calling environment.
- Threadwire is not installed in the image or the project; it remains parent orchestration resolved through unversioned `npx` outside the container.
