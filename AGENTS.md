...
- The dev service mounts the complete `${DEV_HOME_PATH:-/home/dev}` development home at `/home/dev`, the required `${GH_CONFIG_SOURCE_PATH}` GitHub CLI config read-only at `/home/dev/.config/gh`, and Hermes' external `/opt/hermes-dind-shared/auth/provider-runtime` contract. A UID/GID-1000 entrypoint links `.codex`, `.local/share/opencode`, `.opencode`, and `.kimi-code` from the development home to that shared runtime. Do not add SSH keys, individual provider credential mounts, credentials in Compose values, or container-side ownership fixups.
